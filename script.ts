...
let messi: number = 1;
let jugoMessi:boolean = true;
let motivo:string = 'Porque no jugò Messi';

const jugar=(equipoA: number,equipoB: number, juegaMessi:boolean):string=>{
    if(juegaMessi) {
        equipoA += messi
        motivo = `Porque Messi hizo ${messi} Gol/s`
    };
    if(equipoA == equipoB) return `Empate ${motivo}`;
